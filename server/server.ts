...
if (!API_KEY) {
  ...
  console.error('Please create a .env file or set the API_KEY environment variable');
  process.exit(1);
}
...
app.post('/api/convert-scad', authenticateApiKey, async (req, res) => {
  ...
  try {
    ...
    console.log(`"${offContent.substring(0, 300)}${offContent.length > 300 ? '...' : ''}"`);
    ...
    if (polyhedron.vertices.length > 0) {
      console.log(`First vertex: (${polyhedron.vertices[0].x}, ${polyhedron.vertices[0].y}, ${polyhedron.vertices[0].z})`);
    }
    if (polyhedron.colors.length > 0) {
      console.log(`First color: r=${polyhedron.colors[0].r}, g=${polyhedron.colors[0].g}, b=${polyhedron.colors[0].b}`);
    }
    ...
    if (transformedPolyhedron.vertices.length > 0) {
      console.log(`First transformed vertex: (${transformedPolyhedron.vertices[0].x}, ${transformedPolyhedron.vertices[0].y}, ${transformedPolyhedron.vertices[0].z})`);
    }
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Convert endpoint: POST http://localhost:${PORT}/api/convert-scad`);
